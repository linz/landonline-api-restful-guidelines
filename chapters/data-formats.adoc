...
You must always provide one of the formats `int32`, `int64`,
or `float`, `double`,  when you define an API property of
JSON type `number` or `integer`.

However we have removed bigint from integer and decimal types as accepted formats, in these instances, there could be some incorrect guessing of the precision.

e.g. specific language types; in Java, for instance, the `number`
type with `decimal` format would normally have been translated into `BigDecimal`, this is now unclear, `integer` type with
